...
def load_odds_movements():
    odds_movements = pd.read_csv('data/ncaaf_odds_movements.csv')
...
def load_games_data():
    ...
    json_files = sorted([f for f in os.listdir(data_dir) if f.endswith(".json") and f.startswith('ncaaf')], reverse=True)
...
app.layout = html.Div([
    html.H1("NCAAF Odds Dashboard", style={'text-align': 'center'}),
    html.Div([
        html.Div([
